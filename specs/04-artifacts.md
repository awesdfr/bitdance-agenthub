...
> Artifact 是 Agent 产出的「可独立预览的产物」：网页 / 代码 / 文档 / 图片 / diff / PPT。**与 Message 解耦**，有独立的生命周期、版本、二次编辑。**修改 ArtifactContent 结构需先讨论。**
...
`write_artifact` 工具当前 `type` 接：`'web_app' | 'document' | 'image' | 'diff' | 'ppt'`。`code_file` 仍不由 LLM 直接创建，避免把大文件内容塞进 DB 或绕过 workspace 文件写入路径。
