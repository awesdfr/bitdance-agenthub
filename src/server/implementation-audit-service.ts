...
function evidenceForSection(sectionNumber: number): string[] {
  ...
  if (sectionNumber === 141) {
    return ['`src/server/workflow-preset-service.ts` defines a built-in workflow preset library for inbox triage, sales reports, code review, meeting notes, competitor research, file organization, and Jianying video delivery. Presets install into real `workflows`, `workflow_nodes`, and `workflow_edges` using `artifact_transform` nodes, and can be run directly through the existing workflow runner. `/api/workflow-presets`, `/api/workflow-presets/:id/install`, `/api/workflow-presets/:id/run`, and Agent Canvas Preset Library controls expose one-click install/run plus deliverable preview. Tests verify all seven scenarios, installed graph shape, customer-visible output contracts, and direct preset execution.']
  }
