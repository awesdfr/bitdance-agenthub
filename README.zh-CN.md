...
> 当前状态：桌面端优先。Web 版与 Electron 桌面版可运行；手机连接、虚拟工作站、VM/RDP 并行桌面等能力暂缓，不作为当前公开版本的交付范围。

## 当前桌面端能力快照

- **电脑端工作台**：用户可以直接输入目标，系统会把可用模型、智能体、Skills、CLI、工具连接和桌面安全状态串起来检查。
- **智能体设置合并**：智能体与智能体工厂已收敛到同一入口，单个智能体旁边可以进入设置，配置模型、技能、MCP、CLI、软件能力、记忆、上下文、权限和交付物要求。
- **编排画布**：支持类似 ComfyUI 的画布拖拽、背景平移、缩放、连线、节点运行状态、节点交付物和客户可见产物提示。
- **模型管理**：支持模型新增、删除、连接测试、网络出口/代理配置、DeepSeek/OpenAI-compatible 等模型配置。
- **技能中心**：SkillsMP 不再用 iframe 强行内嵌，而是通过内置 CLI/API 搜索、安装、发布和分配给智能体。
- **工具连接商店**：把 CLI、MCP、软件能力按“软件商店”方式展示，用户选软件后再看它支持 CLI、MCP 或封装命令。
- **运行现场**：用现场总览、任务队列、任务详情、最近事件和是否需要处理来替代纯技术监控。
- **交付物中心**：按客户能看到的成果组织 Agent 产物，展示客户可读、预览验收、版本历史和打包交付状态。
- **费用分析**：展示每个模型的实际花费、无缓存估算、缓存省钱率、请求均价和可优化模型。
